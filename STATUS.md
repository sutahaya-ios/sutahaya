...
- Codex(たける側): **TOEIC単語20語を`word_bank_toeic.xlsx`へ分離**。銀のフレーズへdifficulty 1・2の8語、金のフレーズへ3・4・5の12語を移し、`word_bank.xlsx`は使い方・中学17語だけに変更。変換スクリプトはTOEICの2シートを通したID採番・単語/ID重複確認、シート別difficulty検証、4ファイルの不足表示へ対応した。変更した2ブック全4シートを目視確認し、`python3 scripts/word_bank_to_json.py`で3つのJSONすべて「変更なし」を確認。一時移行スクリプトは削除済み
- Codex(たける側): **高校英単語183語を担当者別・ターゲット別の作業用xlsxへ再編**。`word_bank_takeru.xlsx`へターゲット1200(difficulty 1・2、27語)とターゲット1400(3、60語)、`word_bank_tokiya.xlsx`へターゲット1900(4・5、96語)を移し、`word_bank.xlsx`は使い方・中学17語・TOEIC20語だけに変更。変換スクリプトは高校3シートを通したID採番・単語/ID重複確認、シート別difficulty検証、3ファイルの不足表示へ対応した。3ブック全6シートを目視確認し、`python3 scripts/word_bank_to_json.py`で3つのJSONすべて「変更なし」を確認。一時移行スクリプトは削除済み
...
2. **英単語のボリューム拡充(2026年8月25日 方針更新)**:`junior_high.json` / `high_school.json` / `toeic.json` へ、たける・トキヤが直接入力する。現220語はテストデータなので順次刷新し、各カテゴリのdifficulty 1〜5を充実させる
...
- 正データは `Resources/junior_high.json` / `Resources/high_school.json` / `Resources/toeic.json`。IDは `jh_####` / `hs_####` / `tc_####`。同じカテゴリ内の単語重複は禁止し、カテゴリをまたぐ重複は許可する。作業用xlsxはgit管理外で、中学は`word_bank.xlsx`、高校は`word_bank_takeru.xlsx`(ターゲット1200/1400)と`word_bank_tokiya.xlsx`(ターゲット1900)、TOEICは`word_bank_toeic.xlsx`(銀のフレーズ/金のフレーズ)へ入力し、`scripts/word_bank_to_json.py`でJSON差分を確認して反映する
