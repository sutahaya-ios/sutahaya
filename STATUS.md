...
- TOKIYA-YAMAMOTO(Codex・たける側代行): **初回ユーザー向けに1問だけの対戦ルール体験を追加**。文字送りを見て分かった瞬間に4択を押し、そのタップが早押しと回答を兼ねることを、実際の対戦と同じ表示部品で体験できる。初回のみ自動表示し、以後も対戦タブ右上の「遊び方」から再表示可能。Priority 0の実機2台検証は相手端末が現在利用できないため保留し、独立して進められるPriority 1を先行した。XcodeGen生成、Simulator向けビルド、全72テスト、iPhone 17 Pro Simulatorで初回画面の表示を確認。**正解・誤答選択後の画面操作と実機表示は未検証**。Firebase・対戦ルール・得点ルールへの変更なし
